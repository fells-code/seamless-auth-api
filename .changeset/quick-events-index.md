...
Index `auth_events` on `(user_id, created_at)` and `(type, created_at)`.
...
Each of those reads scanned the whole table, which gains a row on every authentication
and is never pruned.
...
and the time-to-login self-join uses it on both sides. The migration only adds indexes
